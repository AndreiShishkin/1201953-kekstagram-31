...
const validateHashtags = (value) => {
  ...
  const isValidHashtags = hashtags.every((hashtag) => HASHTAG_REGEX.exec(hashtag) ? HASHTAG_REGEX.exec(hashtag)[0] === hashtag : false);
...
const validateInicial = (validateForm, hashtagsInput, commentInput) => () => {
  ...
  const getErrorHashtags = () => {
    ...
    hashtags.forEach((hashtag) => {
      if (HASHTAG_REGEX.exec(hashtag) ? HASHTAG_REGEX.exec(hashtag)[0] !== hashtag : true) {
